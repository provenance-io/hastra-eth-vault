...
import type { HastraSMBNavEngineV2 } from "../../typechain-types";

/**
 * HastraSMBNavEngineV2 is a thin subclass of HastraNavEngineV2 — adds only a
 ...
 *
 *   1. V1 SMB proxy upgrades to V2 SMB and initializeV2 initialises correctly.
 *   2. Branded name() returns "HastraSMBNavEngineV2".
 ...
 */
describe("HastraSMBNavEngineV2 (smoke)", function () {
  // ── V1 init params ────────────────────────────────────────────────────
  ...
  async function deployV1Fixture() {
    ...
    const V1Factory = await ethers.getContractFactory("HastraSMBNavEngine");
  ...
  async function deployV2Fixture() {
    ...
    const V2Factory = await ethers.getContractFactory("HastraSMBNavEngineV2");
    ...
    }) as unknown as HastraSMBNavEngineV2;
  ...
  describe("Upgrade from V1 → V2", function () {
    ...
    it("leaves maxRate at V1 value when setMaxRate is not called post-upgrade", async function () {
      const { v1, owner, updater, pauser } = await loadFixture(deployV1Fixture);
      const V2Factory = await ethers.getContractFactory("HastraSMBNavEngineV2");
      const v2 = await upgrades.upgradeProxy(await v1.getAddress(), V2Factory, {
  ...
  describe("name()", function () {
    it("returns HastraSMBNavEngineV2", async function () {
      const { v2 } = await loadFixture(deployV2Fixture);
      expect(await v2.name()).to.equal("HastraSMBNavEngineV2");
    });
  });
  ...
  describe("UUPS upgrade", function () {
    it("owner can upgrade to a fresh HastraSMBNavEngineV2 impl", async function () {
      const { v2 } = await loadFixture(deployV2Fixture);
      const Factory = await ethers.getContractFactory("HastraSMBNavEngineV2");
      const upgraded = await upgrades.upgradeProxy(
        await v2.getAddress(),
        Factory
      ) as unknown as HastraSMBNavEngineV2;
      expect(await upgraded.name()).to.equal("HastraSMBNavEngineV2");
    });
