...
describe("YieldVault Compliance (Freeze/Thaw)", function () {
  ...
  it("Should prevent frozen accounts from transferring tokens", async function () {
    const { yieldVault, freezeAdmin, userA, userB } = await loadFixture(
      deployFixture
    );
    ...
    await expect(
      yieldVault.connect(userA).transfer(userB.address, amount)
  ...
  it("Should prevent transferring tokens TO a frozen account", async function () {
    const { yieldVault, freezeAdmin, owner, userA } = await loadFixture(
      deployFixture
    );
    const amount = ethers.parseUnits("100", 6);
    ...
    await expect(
      yieldVault.connect(owner).transfer(userA.address, amount)
    ).to.be.revertedWithCustomError(yieldVault, "AccountIsFrozen");
  });
  ...
  it("Should allow a frozen account to burn tokens (required for redemption completion)", async function () {
    ...
    await expect(
      yieldVault.connect(userA).requestRedeem(ethers.parseUnits("10", 6))
    ).to.be.revertedWithCustomError(yieldVault, "AccountIsFrozen");
  });
  ...
  it("Should allow thawing an account to resume transfers", async function () {
    ...
    await expect(yieldVault.connect(userA).transfer(userB.address, amount)).to
      .not.be.reverted;
  ...
  it("Should prevent non-admins from freezing accounts", async function () {
    ...
    await expect(
      yieldVault.connect(userA).freezeAccount(userB.address)
  ...
  it("Should not affect other accounts when one account is frozen", async function () {
    const { yieldVault, freezeAdmin, userA, userB, owner } = await loadFixture(
      deployFixture
    );
    const amount = ethers.parseUnits("100", 6);
    ...
    await expect(yieldVault.connect(userB).transfer(owner.address, amount)).to
      .not.be.reverted;

    expect(await yieldVault.balanceOf(owner.address)).to.equal(
  ...
  it("Should block completeRedeem payout to a frozen account", async function () {
    ...
    const { yieldVault, usdc, freezeAdmin, userA, owner } = await loadFixture(
      deployFixture
    );
    ...
    await usdc
      .connect(owner)
      .approve(await yieldVault.getAddress(), redeemAmount);
    ...
    await expect(
      yieldVault.connect(owner).completeRedeem(userA.address)
    ).to.be.revertedWithCustomError(yieldVault, "AccountIsFrozen");
  });
  ...
  it("Should block withdrawUSDC when vault is paused", async function () {
    ...
    await expect(
      yieldVault.connect(owner).withdrawUSDC(owner.address, amount)
  ...
  it("Should revert with AddressNotFoundInWhitelistArray on mapping/array desync", async function () {
    ...
    await expect(
      yieldVault.removeFromWhitelist(desynced.address)
    ).to.be.revertedWithCustomError(
      yieldVault,
      "AddressNotFoundInWhitelistArray"
    );
  });
});
